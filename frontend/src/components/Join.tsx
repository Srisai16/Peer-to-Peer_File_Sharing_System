...
const Join = () => {
      ...
      if (typeof ev.data === "string") {
        ...
        if (state.meta) {
          ...
          if (state.receivedBytes === state.meta.size && !state.fileStream && state.buffers.length > 0) {
             ...
             }, 2000); // Wait 2s to see if EOF arrives properly
          }
        }
      }
    };
  ...
  const initiatePeerConnection = useCallback(async (targetId: string) => {
    ...
    peerConnectionRef.current.set(targetId, { pc, dataChannels: dcs });
    for (let i = 0; i < 8; i++) {
        const dc = pc.createDataChannel(`fileTransfer-${i}`, { ordered: true });
  ...
  const handlePeerOffer = useCallback(async (senderId: string, offer: RTCSessionDescriptionInit) => {
    const pc = new RTCPeerConnection({ iceServers: ICE_SERVERS });
    const dcs: RTCDataChannel[] = [];
    peerConnectionRef.current.set(senderId, { pc, dataChannels: dcs });
    pc.ondatachannel = (ev) => {
      const dc = ev.channel;
  ...
  const handleJoinRoom = async (e: FormEvent) => {
    ...
    const pc = new RTCPeerConnection({ iceServers: ICE_SERVERS });
    const dcs: RTCDataChannel[] = [];
    peerConnectionRef.current.set(hostId, { pc, dataChannels: dcs });
    for (let i = 0; i < 8; i++) {
      const dc = pc.createDataChannel(`fileTransfer-${i}`, { ordered: true });
  ...
  const sendFiles = async (peerId: string, files: File[]) => {
    ...
    if (!conn || conn.dataChannels.length === 0) { alert("Not connected to this peer yet"); return; }
